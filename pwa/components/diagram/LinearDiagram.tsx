...
class SVG {
    ...
    static LinearChartSVG(element: BaseType, data: { label: string, value: number }[], margin: {top: number, right: number, bottom: number, left: number}, options : {
        width: number,
        height: number
    }) {
    ...
    static update(data: { label: string, value: number }[]) {
        if(data.length < 2) return;
